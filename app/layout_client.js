...
export default function RootLayout({ children }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex">
            {/* Sidebar Senior */}
            <aside className="w-64 bg-slate-900 h-screen sticky top-0 flex flex-col p-6 shadow-2xl">
                <div className="mb-12">
                    <h1 className="text-2xl font-black text-white italic tracking-tighter">SIS<span className="text-indigo-400 text-3xl">.</span>ADMI</h1>
                </div>

                <nav className="flex-1 space-y-1">
                    {menu.map((item) => (
                        <Link key={item.label} href={item.href}>
                            <div className={`p-4 rounded-xl flex items-center gap-4 transition-all group ${pathname === item.href ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                                }`}>
