...
export default function ProveedoresPage() {
    ...
    }, [expForm.amountUsd, expForm.amountBs, expForm.exchangeRate, bcvRate]);
    ...
    const fetchAll = async () => {
        ...
        setBcvRate(rate);
        setExpForm(prev => ({ ...prev, exchangeRate: rate.toString() }));
    };
    ...
    return (
        <div className="p-4 md:p-10 bg-gray-50 min-h-full font-sans text-slate-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-slate-900 p-6 rounded-[32px] shadow-xl flex items-center justify-between">
                    <div>
                        <p className="text-[8px] font-black text-white/40 uppercase tracking-widest mb-1">Tasa Referencial</p>
                        <p className="text-2xl font-black text-white">Bs. {bcvRate?.toFixed(2) || '—'}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    <div className="xl:col-span-1">
                        <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-gray-100 sticky top-4">
                            <h3 className="text-xl font-black text-slate-800 uppercase mb-8 border-b pb-4 border-gray-50 tracking-tighter">Nuevo Egreso</h3>
                            ...
                            <form onSubmit={handleExpSubmit} className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-3">Monto USD ($)</label>
                                        <input required type="number" step="0.01" placeholder="0.00"
                                            className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none font-black text-slate-800 text-base focus:ring-2 focus:ring-orange-500 transition"
                                            value={expForm.amountUsd} onChange={e => setExpForm({ ...expForm, amountUsd: e.target.value, activeField: 'usd' })} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-3">Tasa Proveedor</label>
                                        <input required type="number" step="0.01" placeholder={bcvRate.toString()}
                                            className="w-full p-4 rounded-2xl bg-slate-100 border-none outline-none font-black text-blue-600 text-base focus:ring-2 focus:ring-blue-500 transition"
                                            value={expForm.exchangeRate} onChange={e => setExpForm({ ...expForm, exchangeRate: e.target.value, activeField: 'rate' })} />
                                    </div>
                                </div>

                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-3">Equivalente en BS.</label>
                                    <input required type="number" step="0.01" placeholder="0.00"
                                        className="w-full p-4 rounded-2xl bg-orange-50 border-none outline-none font-black text-orange-600 text-lg focus:ring-2 focus:ring-orange-500 transition"
                                        value={expForm.amountBs} onChange={e => setExpForm({ ...expForm, amountBs: e.target.value, activeField: 'bs' })} />
                                </div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-3">Método</label>
                                        <select className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none font-bold text-slate-800 text-[10px] uppercase"
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-3">Recibo/Factura</label>
                                        <input placeholder="S/N"
                                            className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none font-bold text-slate-800 text-[10px] uppercase"
                        ...
                        {expenses.map(ex => (
                            <div key={ex._id} className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
                                <div className="flex flex-col md:flex-row md:items-center gap-6 relative z-10">
                                    ...
                                    <div className="text-right shrink-0 border-l border-gray-50 pl-6">
                                    ...
                                    <button onClick={async () => { if (confirm('¿Eliminar egreso?')) { await fetch(`/api/expenses?id=${ex._id}`, { method: 'DELETE' }); fetchAll(); } }}
                                        className="opacity-0 group-hover:opacity-100 p-2 text-gray-300 hover:text-red-500 transition-all absolute top-0 right-0">✕</button>
                                </div>
